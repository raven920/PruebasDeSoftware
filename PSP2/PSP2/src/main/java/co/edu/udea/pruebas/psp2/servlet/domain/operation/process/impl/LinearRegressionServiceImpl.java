...
public class LinearRegressionServiceImpl implements LinearRegressionService {
    ...
    /**
     *
     * Este método debe retornar un arreglo con los 4 valores que se piden para
     * la regresión, dichos valores son B0, B1, Rxy, R2 y DEBEN SER RETORNADOS
     * EN ESE MISMO ORDEN DENTRO DEL ARREGLO
     *
     * @param datos es la lista ligada que tiene los valores sobre los que se
     * hará el cálculo, viene en forma de dupla.
     * @return Un arreglo con los valores de B0, B1, Rxy, R2
     * @throws PSP2Exception cuando existe un problema que imposibilite el
     * cálculo de algun valor
     */
    @Override
    public double[] getLinearRegressionParameters(ListaLigadaConCabeza<Dupla<Double>> datos) throws PSP2Exception {
        ...
        try{
             resultado = new double[]{salidas.get(0), salidas.get(1), salidas.get(2), salidas.get(3)};
        }catch (NullPointerException e){
            throw new PSP2Exception("Some value couldn't be calculated.");
        }
    ...
    private void sumatorias(ListaLigadaConCabeza<Dupla<Double>> datos) {
        ...
        if ((auxiliar = datos.getPrimero()) != null) {
            while (!datos.finDeRecorrido(auxiliar)) {
                datoDelNodo = auxiliar.getObjeto();
    ...
    public Double rxy() throws PSP2Exception {
        ...
        if (cantidad != 0.0) {
            ...
            if (primFactor * segFactor > 0) {
                rxy = numerador / Math.sqrt(primFactor * segFactor);
                return rxy;
            } else {
                throw new PSP2Exception("Fail calculating square root.");
            }
        } else {
            return 0.0;
        }
    ...
    public Double rCuadrado() throws PSP2Exception {
        Double rxy = rxy();
        if (rxy != null) {
            Double rCuadrado = Math.pow(rxy, 2);
            return rCuadrado;
        } else {
            throw new PSP2Exception("Null X x Y.");
